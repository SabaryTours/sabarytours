        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr>
                  <td colSpan={4} className="px-6 py-8 text-center text-gray-500 font-sans">Loading inquiries...</td>
                </tr>
              ) : inquiries.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-6 py-8 text-center text-gray-500 font-sans">No inquiries found.</td>
                </tr>
              ) : inquiries.map((inq) => (
                <tr key={inq.id} className={`hover:bg-gray-50/50 transition-colors group ${inq.status === 'unread' ? 'bg-orange-50/30' : ''}`}>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className={`text-sm font-sans ${inq.status === 'unread' ? 'font-bold text-gray-900' : 'font-semibold text-gray-800'}`}>
                          {inq.name}
                        </p>
                  ...
                  <td className="px-6 py-4 max-w-sm">
                    <p className="text-sm text-gray-600 font-sans line-clamp-2">{inq.message}</p>
                  </td>
