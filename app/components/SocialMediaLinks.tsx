...
import type { ResolvedSocialLink } from "../lib/socialLinks";
import { resolveSocialLinks } from "../lib/socialLinks";

function SocialIcon({ network }: { network: ResolvedSocialLink["network"] }) {
  const common = { className: "w-full h-full", fill: "currentColor" as const, viewBox: "0 0 24 24" };
  switch (network) {
    case "facebook":
      return (
...
type Variant = "footer" | "circleDark" | "blogInline";
...
export default function SocialMediaLinks({
  ...
  const listClass =
    variant === "footer"
      ? "flex flex-wrap gap-3 list-none p-0 m-0"
      : "flex flex-wrap gap-4 list-none p-0 m-0";

  const linkClass =
    variant === "footer"
      ? "block w-6 h-6 text-white hover:text-[#ff5e00] transition-colors"
      : variant === "blogInline"
